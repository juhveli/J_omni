...
class ComfyUIClient {
    ...
    async generateAudio(params) {
        ...
        const sampleRateInv = 1 / audioContext.sampleRate;
        const phaseStepLeft = 2 * Math.PI * freqLeft * sampleRateInv;
        const phaseStepRight = 2 * Math.PI * freqRight * sampleRateInv;

        // Generate a simple sine wave pattern - optimized
        for (let i = 0; i < bufferLength; i++) {
            ...
            leftData[i] = Math.sin(phaseStepLeft * i) * amplitude;
            rightData[i] = Math.sin(phaseStepRight * i) * amplitude;
        }
